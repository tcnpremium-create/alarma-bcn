  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#060B14", paddingBottom: 70 }}>
      ...
      <section style={{
        ...
        {/* Left-side darkening gradient for text contrast */}
        <div style={{ position: "absolute", top: 0, right: 0, bottom: 0, left: 0, zIndex: 2, background: "linear-gradient(105deg, rgba(6,11,20,0.80) 0%, rgba(6,11,20,0.58) 40%, rgba(6,11,20,0.10) 100%)" }} />

        {/* Zona Perimetral Protegida — static badge top-left */}
        <div style={{
          position: "absolute",
          top: 92, left: 24, zIndex: 4,
          ...
          <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: "#E53E3E", flexShrink: 0 }} />
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: 1200, margin: "0 auto", padding: "150px 24px 72px" }}>
          <div style={{ maxWidth: 640 }}>
            ...
            <h1 style={{ fontSize: "clamp(1.9rem, 5vw, 3.2rem)", fontWeight: 900, color: "#FFFFFF", lineHeight: 1.08, maxWidth: 660, margin: "0 0 14px", letterSpacing: "-0.025em" }}>
        ...
        {/* Cámaras Activas 24h — static status card bottom-right */}
        <div style={{
          position: "absolute", bottom: 28, right: 28, zIndex: 4,
          background: "rgba(6,11,20,0.88)",
          border: "1px solid rgba(229,62,62,0.35)",
          borderRadius: 14, padding: "16px 22px",
          backdropFilter: "blur(10px)", maxWidth: 240,
        }}>
      ...
      {/* ── TEXTO PERSUASIVO ── */}
      <section style={{ background: "#060B14", padding: "48px 24px 0" }}>
        <div style={{ maxWidth: 820, margin: "0 auto", textAlign: "center" }}>
          <p style={{ fontSize: "clamp(1rem, 2vw, 1.15rem)", color: "#CBD5E0", lineHeight: 1.8, margin: 0, fontWeight: 500 }}>
            En Premium Tech Security contamos con la mejor promoción del mercado en videovigilancia 4K e Inteligencia Artificial avanzada. Nuestras instalaciones incluyen detección inteligente de personas y vehículos, visión nocturna optimizada en color y acceso total desde tu móvil sin cuotas mensuales.
          </p>
        </div>
      </section>

      {/* ── KITS DE CÁMARAS ── */}
      <CameraKitsGrid onRequestQuote={() => setModalOpen(true)} />
      ...
      <section style={{ background: "#0A1120", padding: "80px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 52 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#E53E3E", letterSpacing: "0.14em", textTransform: "uppercase" }}>Fabricantes líderes</span>
            <h2 style={{ fontSize: "clamp(1.5rem, 3vw, 2.2rem)", fontWeight: 900, color: "#FFFFFF", margin: "10px 0 0", letterSpacing: "-0.025em" }}>Las mejores marcas del mundo</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {BRANDS.map(b => (
              <div key={b.name} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "36px 32px" }}>
                <h3 style={{ fontSize: 22, fontWeight: 900, color: "#FFFFFF", margin: "0 0 14px", letterSpacing: "0.04em" }}>{b.name}</h3>
                <p style={{ fontSize: 13, color: "#64748B", lineHeight: 1.75, margin: "0 0 20px" }}>{b.desc}</p>
      ...
      <section style={{ background: "#060B14", padding: "80px 24px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            ...
            <h2 style={{ fontSize: "clamp(1.4rem, 3vw, 2rem)", fontWeight: 900, color: "#FFFFFF", margin: "10px 0 0", letterSpacing: "-0.025em" }}>Preguntas frecuentes sobre cámaras de seguridad</h2>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {FAQS.map((faq, idx) => (
              <div key={idx} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.07)", overflow: "hidden" }}>
                <button onClick={() => setOpenFaq(openFaq === idx ? null : idx)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 24px", background: "none", border: "none", cursor: "pointer", textAlign: "left" }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: "#F1F5F9", paddingRight: 16, lineHeight: 1.4 }}>{faq.q}</span>
                  {openFaq === idx ? <ChevronUp size={17} color="#E53E3E" style={{ flexShrink: 0 }} /> : <ChevronDown size={17} color="#475569" style={{ flexShrink: 0 }} />}
                </button>
                {openFaq === idx && (
      ...
      {/* ── CTA FINAL ── */}
      <section style={{ background: "#0A1120", padding: "80px 24px", textAlign: "center" }}>
